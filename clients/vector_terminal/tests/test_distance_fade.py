...
def test_intensity_linear_fade_default():
    """Midpoint between NEAR and FAR sits at midpoint between 1.0 and MIN_GLOW."""
    mid = (cfg.NEAR_DIST + cfg.FAR_FADE) / 2
    expected = 1.0 - 0.5 * (1.0 - cfg.MIN_GLOW)
...
def test_intensity_lerp_with_dynamic_bounds():
    """Midpoint check with dynamic bounds reproduces the math."""
    distance_fade.set_bounds(4.0, 20.0)
    expected_at_12 = 1.0 - 0.5 * (1.0 - cfg.MIN_GLOW)  # midpoint
