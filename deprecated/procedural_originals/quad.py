...
class Quad(Mesh):
    ...
    def __new__(
        cls, radius=0.1, segments=8, aspect=1, scale=(1, 1), mode="ngon", thickness=1
    ):
    ...
    def __init__(
        self, radius=0.1, segments=8, aspect=1, scale=(1, 1), mode="ngon", thickness=1
    ):
        ...
        if not Quad._point_placer:
            Quad._point_placer = Entity(
                parent=Quad._corner_maker,
                x=-radius,
                eternal=True,
