...
#define DELTAS_MAX_KILLS_PER_CHUNK 16 /* per-chunk creature death cap (CREATURES_MAX=8 today) */
...
/* Persistent record of a defeated creature, keyed by its deterministic SPAWN
 * position (set at populate time; survives the creature wandering). Used by
 * slice 1b to make kills stick across chunk re-entry — the finite-world rule. */
typedef struct {
    uint8_t spawn_x;
    uint8_t spawn_y;
} CreatureKill;
...
typedef struct {
    ...
    int kill_count;
    CreatureKill kills[DELTAS_MAX_KILLS_PER_CHUNK];
} ChunkDeltas;
...
/* Record a creature kill (keyed by the creature's deterministic spawn
 * position) + atomic file rewrite. The file format adds "K <sx> <sy>" lines;
 * old readers silently skip them (tolerant). */
