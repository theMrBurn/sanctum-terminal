...
class NineSlice(Mesh):
    outset = 0.4  # nineslice textures may have different padding due to baked drop shadows for example, so you can use this to account for that by making the model extend out.

    def __init__(self, entity_scale=Vec2.one, radius=0.5, outset=Default):
        ...
        outset /= (
            1 / radius
        )  # account for different radius so you can change radius without having to change offset
...
if __name__ == "__main__":
    ...
    button_1 = Draggable(
        ...
        position=(0, 0),
        origin=(-0.5, -0.5),
        texture="nineslice_rainbow",
        color=color.white,
    )  # if entity_scale is not provided to NineSlice, use self.scale
