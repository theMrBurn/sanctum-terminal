...
class GameState:
    ...
    def update_grit(self, amount):
        ...
        if self.grit_meter == 100:
            self._add_log(">>> GRIT MAXIMIZED. BURST READY.")
    ...
    def apply_damage(self, raw_dmg):
        ...
        if final_dmg >= self.hp and not self._death_defied:
            self.hp = 1
            self._death_defied = True
            self._add_log("(!) GRIT ACTIVE: SYSTEM STABILIZED AT 1HP.")
            return 0
        self.hp -= final_dmg
    ...
    def process_step(self, symbol):
        ...
        if t_type == "trap":
            ...
            self._add_log("(!) SYSTEM SPIKE: HP REDUCED.")
            return True
        if symbol == "&":
            return "CHALLENGE"
        if t_type == "trigger":
            self.xp += 25 * self.floor
            self._add_log("DATA FRAGMENT RECOVERED.")
        if t_type == "exit":
            self.floor += 1
            self.visited.clear()
            self._death_defied = False
    ...
    def _apply_perception_regen(self):
        ...
        rate = 0.10 + (ratio * 1.2)
        self.regen_pool += rate
        if self.regen_pool >= 1.0:
            val = int(self.regen_pool)
            if self.hp < self.hp_max:
                self.hp = min(self.hp_max, self.hp + val)
            else:
                self.update_grit(val * 2)
            self.regen_pool -= val

    def get_scaling_stat(self, base):
        return int(base * (1.15**self.floor))
    ...
    def _check_level_up(self):
        if self.xp >= self.xp_next:
            ...
            self.xp_prev = self.xp_next
            self.xp_next = new_target
            growth = 15 + (self.lv * 2)
            self.hp_max += growth
            self.hp = self.hp_max
