...
class ASCIICrawler:
    ...
    def handle_input(self, key):
        ...
        dx, dy = (0, -1) if key=='w' else (0, 1) if key=='s' else (-1, 0) if key=='a' else (1, 0) if key=='d' else (0, 0)
        ...
        if 0<=nx<32 and 0<=ny<32:
            ...
            if res == "CHALLENGE":
                self.active_challenge = self.encounter_engine.TrapChallenge(self.state.lv)
                self.grid[ny, nx] = "."
            elif res:
                self.state.pos = [nx, ny]
                if sym == "$": self.grid[ny, nx] = "."
                if sym == "X": self._build_floor()
                cb = self.encounter_engine.check_step(self.state.hp)
                if cb: self.active_combat = cb

    def _process_combat_input(self, key):
        c = self.active_combat
        if key == "f":
            _, msg = c.player_attack(self.state.lv); self.state.log.append(msg)
        elif key == "d":
            self.state.is_defending = True; self.state.update_grit(15)
            self.state.log.append("ADOPTED DEFENSIVE STANCE.")
        elif key == "b" and self.state.grit_meter >= 100:
            self.state.grit_meter = 0
            for e in c.enemies: e["hp"] -= (self.state.lv * 18)
            self.state.log.append("BURST: VOXEL SHATTER!")
            c.enemies = [e for e in c.enemies if e["hp"] > 0]
        elif key == "r":
            if random.random() > 0.4:
                self.state.log.append("ESCAPE SUCCESSFUL."); self.active_combat = None; return
            else: self.state.log.append("ESCAPE FAILED.")

        if c.enemies:
            dmg = self.state.apply_damage(c.enemy_turn())
            if dmg > 0: self.state.log.append(f"Took {dmg} DMG.")
            self.state.is_defending = False
        else:
            xp = 35 * self.state.floor; self.state.xp += xp
            self.state.log.append(f"VICTORY. +{xp} XP."); self.active_combat = None
    ...
    def _process_challenge_input(self, key):
        if key == "a":
            if self.active_challenge.attempt(self.state.grit_meter):
                ...
                self.state.xp += reward; self.state.log.append(f"BYPASS SUCCESS. +{reward} XP.")
            else:
                self.state.log.append("BYPASS FAILED!"); self.state.apply_damage(self.state.get_scaling_stat(7))
            self.active_challenge = None
        elif key == "t":
