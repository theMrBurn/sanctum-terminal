...
class VoxelTerminal:
        ...
        while self.crawler.state.is_alive:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if char:
                        self.crawler.handle_input(char)

            self._draw_hud()
            if self.crawler.active_combat:
                self._draw_combat()
            elif self.crawler.active_challenge:
    ...
    def _draw_hud(self):
        ...
        self.screen.blit(
            self.font.render(
                f"F: {s.floor} | LV: {s.lv} | {s.get_elapsed_time()}",
                True,
                (0, 255, 200),
            ),
            (20, 20),
        ...
        self.screen.blit(
            self.font.render(
                f"HP: {s.hp}/{s.hp_max} | SYNC: {sync:.1f}%", True, (0, 255, 100)
            ),
            (20, 50),
        )
        ...
        pygame.draw.rect(
            self.screen, (255, 200, 0), (20, 100, int(s.grit_meter * 2), 8)
        )  # Grit
                    ...
                    if x == px and y == py:
                        ...
                        col = (
                            ...
                            if char == "#"
                                ...
                                (255, 255, 0)
                                if char == "$"
                                else (0, 255, 255) if char == "&" else (0, 255, 0)
                            )
                        )
                    else:
                        col = (150, 150, 150)
                    self.screen.blit(
                        self.font.render(char, True, col),
                        (320 + x_off * 25, 150 + y_off * 25),
                    )
        for i, m in enumerate(self.crawler.state.log[-4:]):
            self.screen.blit(
                self.font.render(f"> {m}", True, (150, 150, 150)), (20, 600 + i * 25)
            )

    def _draw_combat(self):
        self.screen.blit(
            self.font.render("--- BATTLE MODE ---", True, (255, 50, 50)), (400, 150)
        )
        for i, e in enumerate(self.crawler.active_combat.enemies):
            ...
            self.screen.blit(
                self.font.render(txt, True, (255, 255, 255)), (350, 220 + i * 35)
            )
        self.screen.blit(
            self.font.render("(F)ight (D)efend (B)urst (R)un", True, (200, 200, 200)),
            (350, 500),
        )
    ...
    def _draw_challenge(self):
        ...
        self.screen.blit(
            self.font.render(f"CHALLENGE: {c.name}", True, (0, 255, 255)), (350, 300)
        )
        self.screen.blit(
            self.font.render("(A)ttempt Bypass | (T)ake Hit", True, (200, 200, 200)),
            (350, 350),
        )
