    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(base_shape, Mesh):
            ...
            raise ValueError(
                f"base_shape must be Mesh/list/tuple, not {type(base_shape)}"
            )
    ...
    def generate(self):
        ...
        for i in range(1, len(self.path)):
            ...
            if self.look_at:
                ...
                ):  # if the first and last point are the same, make the end math the rotation of the start.
            ...
            for j in range(len(self.curr.children)):
                ...
                if self.color_gradient:
                    from_color = sample_gradient(
                        self.color_gradient, (i - 1) / (len(self.path) - 1)
                    )
                    to_color = sample_gradient(
                        self.color_gradient, (i - 0) / (len(self.path) - 1)
                    )
                    self.colors.extend(
                        [
                            to_color,
...
if __name__ == "__main__":
    ...
    thicknesses = ((1, 1), (0.5, 0.5), (0.75, 0.75), (0.5, 0.5), (1, 1))
    e = Entity(
        model=Pipe(path=path, cap_ends=False, thicknesses=thicknesses), texture="shore"
    )
    color_gradient = [color.magenta, color.cyan.tint(-0.5), color.clear]
