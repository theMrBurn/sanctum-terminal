...
class Terrain(Mesh):
    def __init__(
        self, heightmap="", height_values=None, gradient=None, skip=1, **kwargs
    ):
        ...
        for z in range(h):
            for x in range(w):
                ...
                self.vertices.append(
                    Vec3(
                        (x / (w - 1)) + (centering_offset.x),
                        _height_values[x][z],
                        (z / (h - 1)) + centering_offset.y,
                    )
                )
...
if __name__ == "__main__":
    ...
    terrain_bounds = Entity(
        model="wireframe_cube", origin_y=-0.5, scale=(40, 40, 40), color=color.lime
    )
